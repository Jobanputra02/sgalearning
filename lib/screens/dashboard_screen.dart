...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _loadProgress() async {
    ...
    final pitch = await ModuleProgressService.getPitchProgress(
        user.uid);
    final note = await ModuleProgressService.getNoteProgress(
        user.uid);

    if (widget.user.role == 'student') {
    final analytics = await StudentAnalyticsService
        .compute(UserModel(
          uid:       widget.user.uid,
  ...
  Color get _roleBadgeColor {
    switch (widget.user.role) {
      case 'faculty':   return AppTheme.accentLight;
      case 'admin':     return AppTheme.navy;
  ...
  double _progressFor(String moduleKey) {
    switch (moduleKey) {
      case 'pitch': return _pitchProgress;
      case 'note':  return _noteProgress;
      default:      return 0.0;
    }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: const PreferredSize(
          ...
          child: Divider(
              height: 1, thickness: 1, color: AppTheme.borderColor),
        ),
        actions: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border:
                  Border.all(color: _roleBadgeColor, width: 1),
            ),
            child: Text(
              _roleLabel,
              style: TextStyle(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Divider(
                color: AppTheme.borderColor, thickness: 1),
            const SizedBox(height: 20),
            const Text(
              'Ear Training',
              style: TextStyle(
            ...
            if (widget.user.role == 'faculty' || widget.user.role == 'admin') _StudentsCard(user: widget.user),
            const SizedBox(height: 16),
...
class _PendingBadgeButton extends StatefulWidget {
  ...
  @override
  State<_PendingBadgeButton> createState() =>
      _PendingBadgeButtonState();
}

class _PendingBadgeButtonState
    extends State<_PendingBadgeButton> {
  int _pendingCount = 0;
  ...
  Future<void> _loadPendingCount() async {
    try {
      final students = await StudentManagementService
          .getPendingStudents();
      if (mounted) {
...
class _StudentsCard extends StatefulWidget {
  ...
  @override
  State<_StudentsCard> createState() =>
      _StudentsCardState();
}
  ...
  Future<void> _loadCount() async {
    try {
      ...
      if (widget.user.role == 'admin') {
        students = await StudentManagementService
            .getApprovedStudents();
      } else {
        final all = await StudentManagementService
            .getApprovedStudents();
        students = all
            .where((s) =>
                s.branch == widget.user.branch &&
                s.isActive)
            .toList();
      }
